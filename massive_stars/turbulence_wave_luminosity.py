...
ell = 1
freq = 0.5
ax1.loglog(freqs_96,      np.abs(lum_96[:, ells_96 == ell]),           color=cmap[0], label=r'Re $\sim$ 200',  zorder=6, lw=0.25)
ax1.loglog(freqs_128_low, np.abs(lum_128_low[:, ells_128_low == ell]), color=cmap[1], label=r'Re $\sim$ 400',  zorder=5, lw=0.50)
ax1.loglog(freqs_128,     np.abs(lum_128[:, ells_128 == ell]),         color=cmap[2], label=r'Re $\sim$ 800',  zorder=4, lw=0.75)
ax1.loglog(freqs_256,     np.abs(lum_256[:, ells_256 == ell]),         color=cmap[3], label=r'Re $\sim$ 2000', zorder=3, lw=1.00)
ax1.loglog(freqs_384,     np.abs(lum_384[:, ells_384 == ell]),         color=cmap[4], label=r'Re $\sim$ 4000', zorder=2, lw=1.25)
ax1.loglog(freqs_512,     np.abs(lum_512[:, ells_512 == ell]),         color=cmap[5], label=r'Re $\sim$ 6000', zorder=1, lw=3)
kh = np.sqrt(ell*(ell+1))
...
ax1.text(0.12, 3e28/Lstar, r'$f^{-6.5}$', rotation=0)
ax1.set_xlabel(r'frequency (day$^{-1}$)')
ax1.set_ylabel(r'$L_{\rm wave}/L_{*}$ ($L_{*} = 7.45 \times 10^{37}\,$ erg$\,$s$^{-1}$)')

ax2.loglog(ells_96, lum_96[freqs_96 > freq, :][0,:],                 color=cmap[0], label=r'Re $\sim$ 200',  zorder=6, lw=0.75)
ax2.loglog(ells_128_low, lum_128_low[freqs_128_low > freq, :][0,:],  color=cmap[1], label=r'Re $\sim$ 400',  zorder=5, lw=0.75)
ax2.loglog(ells_128, lum_128[freqs_128 > freq, :][0,:],              color=cmap[2], label=r'Re $\sim$ 800',  zorder=4, lw=0.75)
ax2.loglog(ells_256, lum_256[freqs_256 > freq, :][0,:],              color=cmap[3], label=r'Re $\sim$ 2000', zorder=3, lw=1.00)
ax2.loglog(ells_384, lum_384[freqs_384 > freq, :][0,:],              color=cmap[4], label=r'Re $\sim$ 4000', zorder=2, lw=1.25)
ax2.loglog(ells_512, lum_512[freqs_512 > freq, :][0,:],              color=cmap[5], label=r'Re $\sim$ 6000', zorder=1, lw=3)
...
ax2.text(25, 3e27/Lstar, r'$k_h^4=[\ell(\ell+1)]^2$', rotation=0,ha='left')
