...
with h5py.File('../compressible_re1e4_waves/FT_SH_transform_wave_shells/power_spectra.h5', 'r') as pow_f:
...
for ell in range(64):
    ...
    try:
        ...
        kr = lambda f: np.abs(( ((-1)**(3/4) / np.sqrt(2))\
                          *np.sqrt(-2*1j*kh2 - (omega(f)/chi_rad_min) + np.sqrt((omega(f))**3 + 4*1j*kh2*chi_rad_min*N2plateau)/(chi_rad_min*np.sqrt(omega(f))) )).real)
        ur2 = lambda f: (omega(f)) * (R_gas / Cp) * kr(f) * wave_flux_rcb(f) / N2plateau 
