...
s1 = pg.read_output('./gyre_output/summary.txt')
s2 = pg.read_output('./gyre_output/neg_summary.txt')
...
plt.loglog(np.abs(s1['freq'].real), -s1['freq'].imag, lw=0, marker='+')
plt.loglog(np.abs(s2['freq'].real), -s2['freq'].imag, lw=0, marker='x')
plt.show()
