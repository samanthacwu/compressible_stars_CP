...
cax1 = fig.add_axes([0.65, 0.975, 0.30, 0.025])
cax2 = fig.add_axes([0.05, 0.975, 0.30, 0.025])
...
#        plt.show()
    ...
    for j in range(Lmax[i]):
        ...
        if (j+1) % 2 == 0: 
            axl.loglog(freqs, magnitude_cube[j,:], color=even_sm.to_rgba(j+1), lw=0.5)
            axr.loglog(freqs, sum_mag, color=even_sm.to_rgba(Lmax[i]-j), lw=0.5)
        else:
            axl.loglog(freqs, magnitude_cube[j,:], color=odds_sm.to_rgba(j+1), lw=0.5)
            axr.loglog(freqs, sum_mag, color=odds_sm.to_rgba(Lmax[i]-j), lw=0.5)
    axr.loglog(freqs, np.sum(magnitude_cube[:Lmax[i],:], axis=0), c='k', lw=0.25)
...
for ax in [ax2, ax4, ax6]:
    ax.set_ylabel(r'$\sum_{i}^{\ell}\Delta m_{i}\,(\mu\rm{mag})$')
...
plt.savefig('ellsums.png', bbox_inches='tight', dpi=300)
plt.savefig('ellsums.pdf', bbox_inches='tight', dpi=300)
#    plt.clf()
