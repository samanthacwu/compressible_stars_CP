...
with h5py.File('../twoRcore_re3e4_damping/star/star_512+192_bounds0-2L_Re3.00e+04_de1.5_cutoff1.0e-10.h5', 'r') as lumf:
    s_nd = lumf['s_nd'][()]
...
wave_lums_data = dict()
with h5py.File('../twoRcore_re1e4_damping/wave_flux/wave_luminosities.h5', 'r') as lum_file:
    radius_str = '1.25'
...
with h5py.File(output_file, 'r') as out_f:
    ...
    for j in range(1,Lmax+1):
        ...
        with h5py.File('eigenvalues/transfer_ell{:03d}_eigenvalues.h5'.format(j), 'r') as ef:
        ...
        if use_fit:
            surface_s1_amplitude = np.abs(transfer_interp(transfer_freq_hz))*np.sqrt(wave_lum(transfer_freq_hz, j))
            axs[j-1].loglog(transfer_freq, surface_s1_amplitude, c='orange', label='transfer (Eqn 69)', lw=0.5)
        else:
            ...
            surface_s1_amplitude = np.abs(transfer_interp(transfer_freq_hz))*np.sqrt(data_wave_lum(transfer_freq_hz))
            axs[j-1].loglog(transfer_freq, surface_s1_amplitude, c='orange', label='transfer (Raw Wave Lum)', lw=0.5)
    ...
    if use_fit:
        plt.savefig('wavepropagation_transferVerification_fit.png', bbox_inches='tight', dpi=300)
        plt.savefig('wavepropagation_transferVerification_fit.pdf', bbox_inches='tight', dpi=300)
    else:
        plt.savefig('wavepropagation_transferVerification_raw.png', bbox_inches='tight', dpi=300)
        plt.savefig('wavepropagation_transferVerification_raw.pdf', bbox_inches='tight', dpi=300)
    plt.clf()
