...
for file in files:
    ...
    for i, ev in enumerate(evalues):
        ...
        ax1.plot(r, rho**(1/2)*r**(1/2)*N2_scale**(-1/4)*efs_p[i,:].real, label='real')
        ax1.plot(r, rho**(1/2)*r**(1/2)*N2_scale**(-1/4)*efs_p[i,:].imag, label='imag')
        ...
        wave_lum = 4*np.pi*r**2 * rho * np.conj(efs_u[i,2,:]) * efs_p[i,:]
