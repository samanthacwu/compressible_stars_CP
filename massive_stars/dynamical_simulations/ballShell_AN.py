...
def make_fields(bases, coords, dist, vec_fields=[], scalar_fields=[], vec_taus=[], scalar_taus=[], vec_nccs=[], scalar_nccs=[]):
...
def fill_structure(bases, dist, variables, vec_nccs=[], scalar_nccs=[]):
    max_dt = None
    t_buoy = None
    t_rot = None
    for basis_number, bn in enumerate(bases.keys()):
...
variables, timescales = fill_structure(bases, dist, variables, vec_nccs=vec_nccs, scalar_nccs=scalar_nccs)
max_dt, t_buoy, t_rot = timescales
# Put nccs and fields into locals()
locals().update(variables)

def build_analestic_equations(bases, bases_keys):
    problem_variables = []
    problem_taus = []
...
prob_variables = build_analestic_equations(bases, bases_keys)
...
try:
    while solver.proceed:
        ...
        if solver.iteration % 10 == 0 or solver.iteration <= 10:
            ...
            KE_shell = logger_handler.fields['KE_S']
