...
lift_shell_k1_basis = shell_basis.clone_with(k=1)
lift_shell_k2_basis = shell_basis.clone_with(k=2)
liftS_k1   = lambda A: d3.Lift(A, lift_shell_k1_basis, -1)
liftS_k2   = lambda A, n: d3.Lift(A, lift_shell_k2_basis, n)

formulation_k2 = True
condition = False 
if formulation_k2:
    logger.info('using k2 formulation')
    BC_u_S = liftS_k2(tau_u_Stop, -1) + liftS_k2(tau_u_Sbot, -2)
    BC_s1_S = liftS_k2(tau_s_Stop, -1) + liftS_k2(tau_s_Sbot, -2)
    grad_u_S = d3.grad(u_S)
    grad_s1_S = d3.grad(s1_S)
    if condition:
        divU_S = d3.div(u_S) 
    else:
#        divU_S = d3.div(u_S) + d3.dot(er_S_LHS, liftS_k2(tau_u_Stop, -1))
        divU_S = d3.div(u_S)
else:
    logger.info('using k1 formulation')
    BC_u_S = liftS_k1(tau_u_Stop)
    BC_s1_S = liftS_k1(tau_s_Stop)
    grad_u_S = d3.grad(u_S) + er_S_LHS*liftS_k1(tau_u_Sbot)
    grad_s1_S = d3.grad(s1_S) + er_S_LHS*liftS_k1(tau_s_Sbot)
...
if condition:
    ...
    problem.add_equation("divU_B + dot(u_B, grad_ln_ρ_B) = 0", condition="nθ != 0")
    problem.add_equation("divU_S + dot(u_S, grad_ln_ρ_S) = 0", condition="nθ != 0")
...
#    problem.add_equation("divU_B + dot(u_B, grad_ln_ρ_B) + tau_p_S + dot(er_B_LHS, liftB(tau_u_B)) = 0", condition="nθ == 0")
    problem.add_equation("divU_B + dot(u_B, grad_ln_ρ_B) + tau_p_S = 0")
