...
def set_anelastic_problem(problem, bases, bases_keys, stitch_radii=[]):
    ...
    for basis_number, bn in enumerate(bases_keys):
        ...
        if config.numerics['equations'] == 'AN_HD':
            ...
            equations['energy_{}'.format(bn)] = "g_phi*(dt(s1_{0}) + dot(u_{0}, grad_S0_{0})) - div_rad_flux_{0} + taus_s_{0} = - g_phi*dot(u_{0}, grad_s1_{0}) + H_{0} + nu_diff_{0}*VH_{0}".format(bn)
            ...
            equations['energy_{}'.format(bn)] = "g_phi*(dt(s1_{0}) + dot(u_{0}, grad_S0_{0})) - div_rad_flux_{0} + taus_s_{0} = 0".format(bn)
