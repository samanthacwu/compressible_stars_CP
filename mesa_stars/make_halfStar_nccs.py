...
grad_s_field['g'][2] = grad_s_interp
grad_s_field['c'][:,:,:,N:] = 0
grad_s_field['g'][2] *= zero_to_one(rg, 0.995*(L_CZ/L).value, width=width*(L_CZ/L).value)
grad_s_field['c'][:,:,:,-1] = 0
...
print(np.abs(grad_s_field['g'][0]).max())
print(np.abs(grad_s_field['g'][1]).max())



plt.figure()
plt.plot(rg.flatten(), np.abs(grad_s_field['g'][2].flatten()))
